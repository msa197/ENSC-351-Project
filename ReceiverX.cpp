...
//% Student 1 #: 301274460
//% Student 1 userid (email): jkkalsi
//
//% Student Name 2: Mohit Sharma
//% Student 2 #: 301274887
//% Student 2 userid (email): msa197
//
...
void ReceiverX::clearCan()
{
    ...
    int byteRec;
    do{
        byteRec=PE(myReadcond(mediumD, &character, sizeof(character), sizeof(character), TM_2CHAR, TM_2CHAR));
    }while(character==CAN && byteRec);
...
void ReceiverX::purge()
{
    ...
    while(true)
    {
        returnVal=PE(myReadcond(mediumD, &rcvByte, 20, 1, TM_CHAR, TM_CHAR));
        if(returnVal==0)
        {
            break;
        }
